...
public class EqualityMetamorphicTestResult extends TestResult{
    private static final Logger logger = LogManager.getLogger(SubSetMetamorphicTestResult.class);

    public EqualityMetamorphicTestResult(){
    ...
    public boolean isInconsistentSubSet(TestSequence testSequence){
        ...
        Gson gson = new Gson();
        if(source == null){
            String errorMsg = "EqualityMetamorphicTestResult's source is null";
            logger.error(errorMsg);
            throw new RuntimeException(errorMsg);
        }
        ...
        if(metamorphic == null){
            String errorMsg = "EqualityMetamorphicTestResult's metamorphic is null";
            logger.error(errorMsg);
            throw new RuntimeException(errorMsg);
        }
